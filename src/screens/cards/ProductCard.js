...
const ProductCard = (props) => {
  const { imageUrl, name, price, rating, status } = props;
  ...
  return (
    <View style={[styles.catcontain, styles.shadowProp]}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
        ...
        <Image source={{ uri: imageUrl }} resizeMode="center" style={styles.catimg} />
        </View>
        <View style={{ margin: 25 }}>
          <Text style={styles.txtbrd}>{name.length > 15 ? `${name.substring(0, 7)}...` : name}</Text>
        ...
        <TouchableOpacity style={styles.btnptd} onPress={handleAddToCart}>
